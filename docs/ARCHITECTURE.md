...
Lifecycle control is a two-phase boundary. PostgreSQL first commits a `cancelled`, `expired`, or internal `deleted` state, expires active attempts when applicable, appends a provider-neutral audit event, and creates cleanup jobs. Only then does the service delete exact provider objects/versions. Completed evidence receives `evidence_expires_at = completed_at + 30 days`; a startup-and-hourly backend sweep changes due records to `expired` before deletion. Failed calls remain pending for a later sweep or owner-authorized retry, while list/detail/content authorization already denies access. Metadata correction is allowed independently of evidence bytes and is always captured as a before/after audit event without provider references.
