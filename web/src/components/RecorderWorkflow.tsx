      ...
      <CardContent className="p-0">
        <form ref={formRef} onSubmit={(event) => void start(event)}>
          <div className="grid lg:grid-cols-[minmax(0,.78fr)_minmax(0,1.22fr)]">
            ...
            <section className="p-4 sm:p-7" aria-label="Add evidence">
              <div className="mb-3 flex items-start justify-between gap-4 sm:mb-5"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary"><Image aria-hidden="true" className="size-4" /></span><div><h3 className="font-semibold" id="evidence-files-heading">{t('Evidence files')}</h3><p className="mt-1 hidden text-sm text-muted-foreground sm:block">{t('Make sure labels, seals and package condition are clearly visible.')}</p></div></div>{items.length > 0 && !activityId && <Button size="sm" type="button" variant="ghost" onClick={() => { setItems([]); if (fileInputRef.current) fileInputRef.current.value = '' }}><Trash2 aria-hidden="true" className="size-3.5" /> {t('Clear all')}</Button>}</div>
