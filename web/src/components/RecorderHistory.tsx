...
export function RecorderHistory({ refreshKey = 0 }: RecorderHistoryProps) {
  ...
  const readyAssets = selected?.assets.filter((asset) => asset.status === 'ready') ?? []
    ...
    <Card className="overflow-hidden" aria-labelledby="recorder-history-heading">
      ...
      <CardContent className="p-6">
        ...
        {selected && <article className="mt-6 rounded-2xl border bg-muted/20 p-5" aria-labelledby={`activity-${selected.id}`}>
          ...
          <dl className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-card p-3"><dt className="text-xs text-muted-foreground">Status</dt><dd className="mt-1 text-sm font-semibold capitalize">{selected.status}</dd></div><div className="rounded-xl bg-card p-3"><dt className="text-xs text-muted-foreground">Occurred</dt><dd className="mt-1 text-sm font-semibold">{displayDate(selected.occurredAt)}</dd></div><div className="rounded-xl bg-card p-3"><dt className="text-xs text-muted-foreground">{t('Storage')}</dt><dd className="mt-1 text-sm font-semibold">{selected.storageProvider === 's3' ? t('Application storage') : 'Google Drive'}</dd></div></dl>
          ...
          <div className="mt-4 flex flex-wrap gap-2">{(selected.status === 'draft' || selected.status === 'uploading') && <Button disabled={saving} variant="outline" onClick={() => setConfirmAction('cancel')}>Cancel activity</Button>}{selected.status === 'complete' && <Button disabled={saving} variant="destructive" onClick={() => setConfirmAction('delete')}><Trash2 aria-hidden="true" className="size-4" /> Delete activity</Button>}</div>
